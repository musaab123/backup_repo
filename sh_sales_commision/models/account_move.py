...
class ShAccountMove(models.Model):
    ...
    sales_commission_ids = fields.One2many(
        'sale.commission.analysis', 'move_id', string="Sales Commission")
    ...
    def action_post(self):
                            ...
                            for line in related_sale_commission.product_commission_lines:
                                ...
                                if line.with_commission == 'fix':
                                    if line.based_on == 'product' and line.product_id and self.invoice_line_ids.filtered(lambda x: x.product_id == line.product_id):
                                        commission_per = 0.0
                                        commission_amount = 0.0
                                        for invoice_line in self.invoice_line_ids.filtered(lambda x: x.product_id == line.product_id):
                                        ...
                                        commission_per = 0.0
                                        commission_amount = 0.0
                                        for invoice_line in self.invoice_line_ids.filtered(lambda x: x.product_id.categ_id == line.category_id):
                                            if invoice_line.price_unit > 0.0:
                                                product_price_diff = (
                                                    invoice_line.price_unit)-(invoice_line.product_id.standard_price)
        ...
        return super(ShAccountMove, self).action_post()
